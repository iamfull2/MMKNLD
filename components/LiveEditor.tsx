...
const LiveEditor: React.FC = () => {
    ...
    // History Debouncer Ref
    const saveHistoryRef = useRef<Function | null>(null);
    ...
    useEffect(() => {
        ...
        const canvas = new window.fabric.Canvas(canvasRef.current, {
            ...
            selection: true,
            enableRetinaScaling: window.devicePixelRatio > 1 ? true : false, // Optimize for non-retina
        ...
        // PERFORMANCE: Create debounced history saver
        // Saving huge JSON strings on every drag is the #1 perf killer in Fabric apps
        const debouncedSave = debounce((c: any) => {
            if (!c) return;
            // Limit history depth
            ...
            setHistory(prev => {
                ...
                // Keep only last 20 steps to save memory
                if (newHistory.length > 20) return newHistory.slice(newHistory.length - 20);
                return newHistory;
            });
            // Update step relative to sliced history
            setHistoryStep(prev => Math.min(prev + 1, 19)); 
        }, 800); // Wait 800ms after interaction stops
        
        saveHistoryRef.current = debouncedSave;
        ...
        canvas.on('object:modified', update); // Only modified, not moving
    ...
    const updateLayers = (canvas: any) => {
        // Optimization: Use getObjects('image') if only caring about images, but we need all
        const objs = canvas.getObjects();
        // Limit layer mapping for large canvases to avoid UI freeze
        const mapped = objs.map((o: any, i: number) => ({
            id: i,
            type: o.type,
    ...
    const selectLayer = (index: number) => {
        ...
        const activeObj = objs[objs.length - 1 - index];
        if (activeObj) {
    ...
    const updateProp = (key: string, value: any) => {
        ...
        if (obj) {
            ...
            // Trigger history via debounce (manual call to ensure update)
            if (saveHistoryRef.current) saveHistoryRef.current(fabricCanvas);
        ...
        if (file) {
            ...
            reader.onload = (f) => {
                window.fabric.Image.fromURL(f.target?.result as string, (img: any) => {
                    img.scale(0.3);
                    img.set({ name: file.name, objectCaching: false });
    ...
    const addShape = (type: 'rect' | 'circle') => {
        let shape;
        if (type === 'rect') {
            shape = new window.fabric.Rect({ left: 100, top: 100, width: 100, height: 100, fill: COLORS.primary, name: 'Rectangle', objectCaching: false });
        } else {
            shape = new window.fabric.Circle({ left: 100, top: 100, radius: 50, fill: COLORS.secondary, name: 'Circle', objectCaching: false });
        }
    ...
    const addGeneratedToCanvas = () => {
        ...
        window.fabric.Image.fromURL(genResult.url, (img: any) => {
            img.scale(0.3);
            img.set({ left: 150, top: 150, name: genResult.prompt, objectCaching: false });
            fabricCanvas.add(img);
            fabricCanvas.setActiveObject(img);
            fabricCanvas.requestRenderAll();
            setActiveModal(null);
            ...
            <div className="flex-1 relative bg-nexus-bg flex flex-col">
                ...
                <div className="flex-1 relative overflow-hidden flex items-center justify-center" ref={containerRef}>
                    ...
                    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-nexus-panel border border-nexus-border-light rounded-full px-4 py-2 flex items-center gap-4 shadow-xl">
                        <button onClick={() => { setZoom(Math.max(0.1, zoom * 0.9)); fabricCanvas.setZoom(zoom * 0.9); fabricCanvas.requestRenderAll(); }} className="hover:text-nexus-secondary font-bold text-lg">−</button>
                        <span className="text-xs font-mono w-10 text-center">{Math.round(zoom * 100)}%</span>
                        <button onClick={() => { setZoom(Math.min(5, zoom * 1.1)); fabricCanvas.setZoom(zoom * 1.1); fabricCanvas.requestRenderAll(); }} className="hover:text-nexus-secondary font-bold text-lg">+</button>
                        <button onClick={() => { setZoom(1); fabricCanvas.setZoom(1); fabricCanvas.viewportTransform = [1,0,0,1,0,0]; fabricCanvas.requestRenderAll(); }} className="hover:text-nexus-secondary text-xs font-bold">RESET</button>
                    </div>
                </div>
            </div>
