...
const NexusLogoSmall: React.FC = () => (
    <div className="relative w-20 h-20 mx-auto mb-4 flex items-center justify-center group cursor-pointer perspective-1000">
        <div className="w-full h-full rounded-full bg-gradient-to-br from-slate-200 via-slate-500 to-slate-800 shadow-[0_0_20px_rgba(0,240,255,0.3)] relative z-10 overflow-hidden transform transition-transform duration-700 group-hover:rotate-12">
            ...
            {/* Changed Red to Blue */}
            <div className="absolute top-1 left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-blue-500 rounded-full shadow-[0_0_5px_rgba(59,130,246,0.8)]"></div>
...
const AuthGate: React.FC<AuthGateProps> = ({ children }) => {
    ...
    const handleAuth = async (e: React.FormEvent) => {
        ...
        // 🟢 GOD MODE / ADMIN BYPASS LOCAL (Robust Check)
        // Checks strictly for the provided credentials
        if (email.trim().toLowerCase() === 'mathias2matheus2@gmail.com' && password.trim() === 'MMK200981@@@@') {
            setStatus('verifying');
        ...
        try {
            ...
            if (msg.includes("Invalid login credentials")) {
                setStatus('denied');
                setErrorMsg("Credenciais inválidas. Verifique senha.");
                setTimeout(() => setStatus('idle'), 3000);
                return;
            }
